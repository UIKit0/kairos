...
class SMMailUtilJava {
  ...
  public static void handlePart(Part part) throws MessagingException, IOException {
	...
	public static File saveFile(String filename, InputStream input) throws FileNotFoundException, IOException {
      ...
      File file = new File(filename);
      if (true)
			return file; // Disabled saveFile by Oobe (Victor Kazarinov). Seems like this is for tests or to show images from server directly in web-app. Instead, we should pass full images inside message to client (this will work slow), or provide IDs of parts and let client first show text content and then download in bacground images and other attachements if need.
      for (int i = 0; file.exists(); i++) {
          file = new File(filename + i);
      }
